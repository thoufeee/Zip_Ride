...
func (h *DriverHandler) SendOTP(c *gin.Context) {
	var req sendOTPReq
	...
	if n, err := h.rdb.Incr(ctx, pkey).Result(); err == nil {
		if n == 1 {
			_ = h.rdb.Expire(ctx, pkey, time.Minute).Err()
		}
		if n > 3 {
			utils.Error(c, http.StatusTooManyRequests, "too many requests, try again later")
			return
		}
	}
	if n, err := h.rdb.Incr(ctx, ikey).Result(); err == nil {
		if n == 1 {
			_ = h.rdb.Expire(ctx, ikey, time.Minute).Err()
		}
		if n > 10 {
			utils.Error(c, http.StatusTooManyRequests, "too many requests, try again later")
			return
...
func (h *DriverHandler) VerifyOTP(c *gin.Context) {
	...
	if err := h.db.Where("phone = ?", phone).First(&d).Error; err != nil {
		...
		if !d.IsVerified {
			_ = h.db.Model(&models.Driver{}).Where("id = ?", d.ID).Update("is_verified", true).Error
		}
	}
...
func (h *DriverHandler) RefreshToken(c *gin.Context) {
	var body struct {
