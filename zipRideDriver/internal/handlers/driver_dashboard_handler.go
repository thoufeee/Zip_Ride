...
func RegisterDriverDashboardRoutes(r *gin.Engine, cfg *config.Config, db *gorm.DB, rdb *redis.Client, log *zap.Logger) {
	...
	g.GET("/:driverId/profile", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
		var d models.Driver
		if err := db.First(&d, uid).Error; err != nil {
			utils.Error(c, http.StatusNotFound, "driver not found")
			return
		}
		var v models.Vehicle
		_ = db.Where("driver_id = ?", uid).Order("id desc").First(&v).Error
		d.PasswordHash = ""
		utils.Ok(c, "profile", gin.H{"driver": d, "vehicle": v})
	})

	type patchStatus struct{ IsOnline bool `json:"is_online" binding:"required"` }
	g.PATCH("/:driverId/status", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
		...
		if err := db.Model(&models.Driver{}).Where("id = ?", uid).Update("is_online", req.IsOnline).Error; err != nil {
	...
	g.GET("/:driverId/earnings/summary", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
		...
		_ = db.Model(&models.Earning{}).Where("driver_id = ?", uid).Select("COALESCE(SUM(amount),0)").Scan(&lifetime).Error
		_ = db.Model(&models.Earning{}).Where("driver_id = ? AND created_at >= ?", uid, seven).Select("COALESCE(SUM(amount),0)").Scan(&last7).Error
		_ = db.Model(&models.Earning{}).Where("driver_id = ? AND created_at >= ?", uid, thirty).Select("COALESCE(SUM(amount),0)").Scan(&last30).Error
		var ridesCount int64
		_ = db.Model(&models.Ride{}).Where("driver_id = ?", uid).Count(&ridesCount).Error
		utils.Ok(c, "earnings summary", gin.H{
			"lifetime": lifetime,
			"last7":   last7,
			"last30":  last30,
	...
	g.GET("/:driverId/earnings/trend", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
	...
	g.GET("/:driverId/rides/summary", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
	...
	g.POST("/:driverId/withdraw", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
	...
	g.GET("/:driverId/dashboard", func(c *gin.Context) {
		uid, ok := mustOwn(c)
		if !ok {
			return
		}
		...
		_ = db.Model(&models.Earning{}).Where("driver_id = ? AND created_at >= ?", uid, today).Select("COALESCE(SUM(amount),0)").Scan(&todayEarnings).Error
		_ = db.Model(&models.Ride{}).Where("driver_id = ? AND status = ? AND created_at >= ?", uid, "completed", today).Count(&completedToday).Error
		var d models.Driver
		_ = db.First(&d, uid).Error
		var v models.Vehicle
		_ = db.Where("driver_id = ?", uid).Order("id desc").First(&v).Error
